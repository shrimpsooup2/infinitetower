...
| `npm run record -- run.json` | Video of a game the Strategist recorded (`--record run.json`), played in the real client (`--show` for a cursor, slowed moves and reasons) |
...
tools/          playtest bot, Strategist, game recorder, pregen, screenshots
```
