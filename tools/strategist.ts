...
interface Probe { lives: number; pen: number }

const score = (p: Probe) => p.lives * 60 + p.pen;

/** Play the next wave (or `waves` waves) on a copy of the game after `move`, with enemy HP x`stress`. */
function rollout(ctx: Ctx, base: WorldSave, move: Move | null, stress: number, waves = 1, bound = Infinity): Probe | null {
  ctx.rollouts++;
  ...
  let pen = 0;
  for (let k = 0; k < waves && w.phase !== 'victory' && w.phase !== 'defeat'; k++) {
    if (w.callWave() !== null) break;
    ...
    for (let t = 1; !w.quiescent() && w.phase === 'running' && t < 60 * 300; t++) {
      ...
      if ((lives0 - w.lives) * 60 > bound) return { lives: lives0 - w.lives, pen: 0 };
    ...
    for (const [f, l] of deep.values()) pen += f * f * l;
  }
  return { lives: lives0 - Math.max(0, w.lives), pen };
}
 ...
 */
function bendingPoint(w: World, ctx: Ctx, snap: WorldSave, from: number): { stress: number; base: Probe } {
  let stress = from;
...
function plan(w: World, ctx: Ctx): string[] {
  ...
  for (let step = 0; step < 14; step++) {
    ...
    // The coming wave played at its real HP is exactly what will happen. If it
    // leaks, fixing that comes first; margin for later waves comes after.
    const truth = rollout(ctx, snap, null, 1)!;
    const { stress, base } = truth.lives > 0 ? { stress: 1, base: truth } : bendingPoint(w, ctx, snap, from);
    if (stress > 1) from = stress;
    const s0 = score(base);
    let best: { m: Move; value: number; gain: number } | null = null;
    for (const m of moves) {
      const p = rollout(ctx, snap, m, stress, 1, s0);
      if (!p) continue;
    ...
    done.push(`${describe(best.m, w)} (-${best.gain.toFixed(1)} @x${stress.toFixed(1)})`);
    apply(w, best.m);
  }
  return done;
}
...
export function play(mapId: string, diff: Diff, seed: number, o: { stress?: number; trace?: boolean; record?: boolean } = {}): Result & { script?: Script } {
  ...
  const result: Result = {
    ...
    levels: w.towers.reduce((a, t) => a + t.level - 1, 0), rollouts: ctx.rollouts, ms: Date.now() - t0,
  };
...
const line = (r: Result) =>
  `${r.map.padEnd(12)} ${r.diff.padEnd(7)} ${String(r.seed).padStart(3)}  ${(r.won ? `WON ${r.wave}` : `lost at ${r.wave}`).padEnd(12)} ${String(r.lives).padStart(4)}  ${r.towers.padEnd(12)} ${String(r.fused).padStart(5)} ${String(r.levels).padStart(5)} ${String(r.rollouts).padStart(7)} ${(r.ms / 1000).toFixed(0).padStart(5)}s`;
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (job) {
    ...
    const header = `map          diff    seed  result       lives  towers (t1/t2/t3) fused  lvls  rollouts   time`;
...
function summary(results: Result[], maps: string[], diffs: Diff[]): void {
  console.log(`\nbest over seeds (W = won, number = wave reached):`);
  console.log(`${'map'.padEnd(12)} ${diffs.map((d) => d.padStart(8)).join('')}`);
  for (const m of maps) {
    const cells = diffs.map((d) => {
      const rs = results.filter((r) => r.map === m && r.diff === d);
      if (!rs.length) return '-'.padStart(8);
      const best = rs.reduce((a, b) => (b.won && !a.won) || (b.won === a.won && b.wave > a.wave) ? b : a);
      return (best.won ? `W${best.wave}` : String(best.wave)).padStart(8);
    });
